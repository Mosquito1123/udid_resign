...
def glt_parser_args(argv):
    ...
    try:
        shortParamters = "Vhi:n:b:d:m:o:e:l"
        longParamters = ["version", "help", "input", "name", "bundleid", "developer", "mobile", "output", "encrypt",
                         "codesignid"]
...
def glt_handle_outputName():
    ...
    if glt_exportPath == '':
        glt_exportPath = os.path.join(os.path.dirname(__file__),"glt_output.ipa")
...
def glt_valid_ipa():
    ...
    mobileprovision = os.path.join(payload_dir,'Payload/%s/embedded.mobileprovision' % app_name)
    print(mobileprovision)
    provisioned_devices = 'openssl smime -inform der -verify -noverify -in "%s" | xmlstarlet sel -t -v "/plist/dict/key[. = "ProvisionedDevices"]/following-sibling::array[1]/string[1]"' % mobileprovision
    print(glt_cmd(provisioned_devices))
    result = glt_cmd(provisioned_devices).strip()

    team_name = 'openssl smime -inform der -verify -noverify -in "%s" | xmlstarlet sel -t -v "/plist/dict/key[. = "TeamName"]/following-sibling::string[1]"' % mobileprovision
    print(glt_cmd(team_name))
    result2 = glt_cmd(team_name).strip()
    ...
    if result == "":
        ...
        if result2 != "":
            ...
            print('该 mobileprovision 没有合法TeamName')
            print(glt_cmd('openssl smime -inform der -verify -noverify -in "%s"' % mobileprovision))
            return False

    else:
        print('该 mobileprovision 是 Ad Hud 应用 或者是 开发者版应用')
        print(glt_cmd('openssl smime -inform der -verify -noverify -in "%s"' % mobileprovision))
        return False
...
if __name__ == "__main__":
	...
	if re.match(r'^https?:/{2}\w.+$', encrypt):
            # print('web')

            glt_source = os.path.join(os.path.dirname(__file__),encrypt.split('/')[-1])
            # print('wt_input路径：%s' % glt_source)
            glt_handle_web_source(encrypt)
        else:
	    glt_source = encrypt
        glt_encrypt = encrypt
        glt_handle_source(glt_source)
